 ...
 * SPMC_SHARED_MEMORY_OBJ_SIZE in platform_def.h to use a different value.
 */
#ifndef SPMC_SHARED_MEMORY_OBJ_SIZE
...
struct spmc_shmem_obj_state {
	uint8_t data[SPMC_SHARED_MEMORY_OBJ_SIZE];
	size_t allocated;
	uint64_t next_handle;
	struct spinlock lock;
};

static struct spmc_shmem_obj_state spmc_shmem_obj_state = {
